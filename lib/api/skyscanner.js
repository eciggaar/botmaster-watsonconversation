            ...
            client.get(poll_url, args, function(data, response) {
              ...
              if (data.Status == 'UpdatesComplete') {
                ...
                if (data.Itineraries[0] === undefined) {
                  ...
                  callback({text: [myLocalize.translate('We\'ve found a flight for you for ') + '€' + Math.round(price) + '. :-)',
                                        myLocalize.translate('Outbound flight: ') + outboundCarrierCode + outboundFlightNumber + myLocalize.translate(' on ') + outboundDepature.substring(0,10) + ', ' + params.origin + ' ' + outboundDepature.substring(11,16) + ' -> ' + params.destination + ' ' + outboundArrival.substring(11,16) + '  (' + outboundOperatorCarrierName + ')',
                                        //myLocalize.translate('Operated by: ') + outboundOperatorCarrierName,
                                        myLocalize.translate('Inbound flight: ') + inboundCarrierCode + inboundFlightNumber + myLocalize.translate(' on ') + inboundDepature.substring(0,10) + ', ' + params.destination + ' ' + inboundDepature.substring(11,16) + ' -> '+ params.origin + ' ' + inboundArrival.substring(11,16) + '  (' + inboundOperatorCarrierName + ')',
                                        //myLocalize.translate('Operated by: ') + inboundOperatorCarrierName,
                                        myLocalize.translate('This service is powered by IBM Watson and Skyscanner.'),
                                        myLocalize.translate('Go to http://skyscanner.net to actually book this flight.'),
